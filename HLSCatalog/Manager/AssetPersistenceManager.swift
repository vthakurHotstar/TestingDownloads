...
class AssetPersistenceManager: NSObject {
    ...
    let requestTypeKey = "x-request-type"

    let avHeaderFieldKey = "AVURLAssetHTTPHeaderFieldsKey"
    ...
    override private init() {
        ...
        headers[userAgentKey] = "Disney+;com.disneyplus.mea/22.04.27.100 (ios/13.3)"
        headers[requestTypeKey] = "download"
        #if DEBUG
            headers["X-AllowRequest"] = "true"
        #endif
    ...
    func downloadStream(for asset: Asset) {
        ...
        options[avHeaderFieldKey] = headers
        let urlAsset = AVURLAsset(url: asset.urlAsset.url, options: options)
...
extension AssetPersistenceManager: AVAssetDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, aggregateAssetDownloadTask: AVAggregateAssetDownloadTask,
        ...
        print("Progresss :: ",percentComplete)
