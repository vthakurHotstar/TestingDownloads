...
public typealias AppCertificateCompletionHandler = (Data?) -> Void
...
class ContentKeyDelegate: NSObject, AVContentKeySessionDelegate {
    ...
    func requestApplicationCertificate(handler: @escaping (AppCertificateCompletionHandler)) {
        let userAgentForHeader : String? = "Disney+;com.disneyplus.mea/22.04.27.100 (ios/13.3)"
        let cert = "https://sub-media.hotstar.com/atv-player/path/hotstar_certificate.cer"
        if let url = URL(string: cert) {
            var urlRequest = URLRequest(url: url, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 8)
            if let userAgentForHeader = userAgentForHeader {
                urlRequest.addValue(userAgentForHeader, forHTTPHeaderField: "User-Agent")
            }
            urlSession.dataTask(with: urlRequest) { certificateData, urlResponse, error in
                handler(certificateData)
            }.resume()
        }
    }
    ...
    lazy var urlSession: URLSession = {
        ...
        sessionConfiguration.httpAdditionalHeaders = ["User-Agent": "Disney+;com.disneyplus.mea/22.04.27.100 (ios/13.3)"]
        sessionConfiguration.httpMaximumConnectionsPerHost = 30
        let session = URLSession(configuration: sessionConfiguration)

        return session
    }()
    
    func requestContentKeyFromKeySecurityModule(spcData: Data, assetID: String, handler: @escaping (AppCertificateCompletionHandler)) {
        
        // MARK: ADAPT - You must implement this method to request a CKC from your KSM.
        
        let licenceUrl : String? = "https://ipl.service.expressplay.com/hms/fp/rights/?ExpressPlayToken=BgAAABNlKesAJDIxNmZiODg3LWZjNTUtNGJiNS1iMTg1LTA4MjFiMDk1MjhjNQAAAGAhePrSYENgoNQvss999rPsClo_-OEHQls6hzkmMjXdHoQBLxZyxsNnB_Hxtx8oOKd2ZR_0lT5-LPksqWCQqg3eip7RSL7rIhNwtZEXBIuNisV_EixayRnjXHKTmzTxJfxMhbkUM1UrsazAiFplMicTRj4-Jg"
        if let license = licenceUrl, let url = URL(string: license) {
            ...
            request.setValue("\(spcData.count)", forHTTPHeaderField: "Content-Length")
            let dataTask = urlSession.dataTask(with: request) { responseData, response, error in
    ...
    func handleStreamingContentKeyRequest(keyRequest: AVContentKeyRequest) {
        ...
        let provideOnlinekey: () -> Void = { () -> Void in
            ...
            do {
                self.requestApplicationCertificate(handler: { applicationCertificate in
                    ...
                    let completionHandler = { [weak self] (spcData: Data?, error: Error?) in
                        ...
                        do {
                            ...
                            strongSelf.requestContentKeyFromKeySecurityModule(spcData: spcData, assetID: assetIDString, handler: { ckcData in
